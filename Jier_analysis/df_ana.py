...
def plot_timeseries(y, timesteps=None, selyears=None, title=None, ax=None):
    ...
    if timesteps is None:
        ...
        # print(np.where(n_of_times == n)[0][0], np.where(n_of_times == n)[0], where[np.where(n_of_times == n)[0][0] ])
        # print(where, n_of_times, cutoff, sep='\n\t\n')
        # sys.exit()
...
def build_ts_matric(df_init, win=20, lag=0, columns=list, rename=dict, period='fullyear'):
    ...
    win -> resample_df make resampling attribute
    rename -> rename columns attribute function
    period -> selperiod attribute function: based on RV_mask or select part of year based on start_end_date ('06-01', '08-31')'
    plot functie, specifier columns from df
    '''
