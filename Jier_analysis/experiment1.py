...
for i in range(len(cols[1:])):
    ...
    # ar, const = inv.extract_ar_data(rg_data, cols[i+1])

    # poly_prec = inv.polynomial_fit(ar=ar, rg_data=rg_data, col=cols[i + 1], sigma=np.std(rg_data[cols[i+1]].values), const=const, dependance=False)
    ...
    # result_var_prec = inv.wavelet_variance_levels(data=rg_data, col=cols[i+1], wavelet=wave, mode=mode,  levels=6)

    # prec_cA = inv.create_wavelet_details(poly_prec, index_poly=rg_index, level=3, wave=wave, mode=mode , debug=True)
    # target_cA = inv.create_wavelet_details(poly_dep, index_poly=rg_index, level=3, wave=wave, mode=mode , debug=True)
    ...
    # # prec_lag = inv.get_mci_coeffs_lag(details_prec=prec_cA, details_target=prec_cA, index=rg_index, rg_obj=rg_obj_aggr, debug=False)

    # inv.plot_mci_prediction(detail_prec=prec_cA, prec_lag=target_prec_lag, title='Dependance', savefig=True)
    # inv.plot_wavelet_variance(var_result=result_var_target, title=6, savefig=True)
    # inv.plot_wavelet_variance(var_result=result_var_prec, title=6, savefig=True)
    # inv.display_polynome(poly=poly_dep, ar_list=[ar_ts[0], ar_ts[1], gamma], rg_data=rg_data, col=cols[0], title=f'AR fit dependance {gamma} gamma', save_fig=True, dependance=True)
    ...
    # inv.display_polynome(poly=poly_prec, ar_list=ar, rg_data=rg_data, col=cols[i + 1], title=f'AR fit on {cols[i+1]} precursor', save_fig=True, dependance=False)
    # # plt.show()