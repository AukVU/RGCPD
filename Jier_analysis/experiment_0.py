...
for col in cols[1:]:
    ...
    # for it in np.arange(0, 6):
        ...
        # result_var_target = inv.wavelet_variance_levels(data=pd.Series(poly_ts, index=rg_index), wavelet=wave, mode=mode, levels=6)
        # result_var_prec_dep = inv.wavelet_variance_levels(data=pd.Series(poly_dep, index=rg_index),  wavelet=wave, mode=mode,  levels=6)
        # result_var_prec = inv.wavelet_variance_levels(data=pd.Series(poly_prec, index=rg_index),  wavelet=wave, mode=mode,  levels=6)

        _, prec_cD = inv.create_wavelet_details(poly_prec, index_poly=rg_index, level=15, wave=wave, mode=mode , debug=True)
        _, target_cD = inv.create_wavelet_details(poly_dep, index_poly=rg_index, level=15, wave=wave, mode=mode , debug=True)
        ...
        inv.plot_mci_prediction(detail_prec=prec_cD, prec_lag=target_prec_lag, title=f'Dependance with {str(np.round(nu, 2))} variation with precursor {col}', savefig=False)
        ...
        # inv.plot_wavelet_variance(var_result=result_var_target, title=cols[0], savefig=False)
        # inv.plot_wavelet_variance(var_result=result_var_prec, title=col, savefig=False)
        # inv.plot_wavelet_variance(var_result=result_var_prec_dep, title='dependance', savefig=False)
        
        inv.display_polynome(poly=poly_dep, ar_list=[ar_ts[0], ar_ts[1], 1.0], rg_data=rg_data, col=cols[0], title=f'target {cols[0]} and precursor {col} with nu {str(np.round(nu, 2))} and {1} gamma', save_fig=False, dependance=True)
        # inv.display_polynome(poly=poly_ts, ar_list=ar_ts, rg_data=rg_data, col=cols[0], title=f'AR fit on {cols[0]} target with iteration', save_fig=False, dependance=False)
        # inv.display_polynome(poly=poly_prec, ar_list=ar, rg_data=rg_data, col=col, title=f'AR fit on {col} precursor with {str(nu)} variation ', save_fig=False, dependance=False)
        plt.show()
