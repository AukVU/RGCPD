...
from class_find_precursors import Precursors
...
class RGCPD2:
    ...
    def __init__(self, list_of_name_path=None, list_for_EOFS=None, 
        ...
        self.precur = Precursors(verbosity=self.verbosity, )
        print(f"Outdic_precur = {self.precur.outdic_precur}")
    ...
    def traintest(self, method='no_train_test_split', seed=1,
        ...
        self.precur.method = method
        self.precur.seed = seed
        self.precur.kwrgs_events = kwrgs_events
        self.precur.precursor_ts = self.import_prec_ts



        TV, self.df_splits = self.precur.RV_and_traintest(self.fullts,
                                                         self.TV_ts)
        self.TV = TV
        self.path_outsub1 = os.path.join(self.path_outsub0,
                                      '_'.join([self.precur.method,
                                                's'+ str(self.precur.seed)]))
    ...
    def calc_corr_maps(self, alpha=0.01, FDR_control=True):
        ...
        self.precur.alpha = alpha
        self.precur.lags = self.lags
        self.precur.FDR_control = FDR_control
        self.precur.kwrgs_load = self.kwrgs_load
        self.precur.list_precur_pp = self.list_precur_pp

        self.outdic_precur = self.precur.calculate_corr_maps()
    ...
    def cluster_regions(self, distance_eps=700, min_area_in_degrees2=2,
        ...
        self.precur.distance_eps = distance_eps
        self.precur.min_area_in_degrees2 = min_area_in_degrees2
        self.precur.group_split = group_split

        for name, actor in self.outdic_precur.items():
            actor = self.precur.cluster_DBSCAN_regions(actor)
    ...
    def get_ts_prec(self, precur_aggr=None):
        ...
        self.precur.precur_aggr = self.precur_aggr
        ...
        if precur_aggr is not None:
            ...
            self.fulltso, self.TV_tso, inf = functions_pp.process_TV(self.fulltso,
                                                              ...
                                                              self.start_end_date,
                                                              self.start_end_year)
            TV, df_splits = self.precur.RV_and_traintest(self.fulltso, 
                                             self.TV_tso)
        else:
            # use original TV timeseries
            TV = self.TV ; df_splits = self.df_splits
        
        self.precur.TV = TV; self.precur.df_splits = df_splits
        ...
        if hasattr(self, 'outdic_precur'):
            if self.outdic_precur is not None:
                ...
                get_ts = self.precur.get_prec_ts
                self.outdic_precur = get_ts()
                                                                
        else:
            self.outdic_precur = None
            self.precur.outdic_precur = None


        if self.outdic_precur is not None:
            # if spatial precursors extracted, create df for timeseries
            self.df_data = self.precur.df_data_prec_regs()

        # Append (or only load in) external timeseries
        if self.import_prec_ts is not None:
            self.df_data_ext = self.precur.import_precur_ts(self.import_prec_ts,
                                                             self.precur_aggr,
                                                             self.start_end_date,
                                                             self.start_end_year)
    ...
    def PCMCI_df_data(self, path_txtoutput=None, tau_min=0, tau_max=1,
        ...
        if path_txtoutput is None:
            self.params_str = '{}_at{}_tau_{}-{}_conds_dim{}_combin{}_dt{}'.format(
                          pc_alpha, self.precur.alpha, tau_min, tau_max, 
