...
class Precursors:
    ...
    def RV_and_traintest(self, fullts, TV_ts, method=str,
        ...
        self.TV = RV_class(df_fullts, df_RV_ts, kwrgs_events)
        self.TV.name = None #TODO
        ...
        if precursor_ts is not None:
            ...
            self.df_splits = functions_pp.rand_traintest_years(TV, method=method,
                                                            seed=seed, 
    ...
    def calculate_corr_maps(self, kwrgs_load,
                            list_precur_pp=list, lags=np.array([1]), 
                            alpha=0.05, FDR_control=True): #TV, df_splits #TODO
        '''
        ...
        self.outdic_precur = dict()
        class act:
            def __init__(selfInner, name, corr_xr, precur_arr):
                selfInner.name = name
                selfInner.corr_xr = corr_xr
                selfInner.precur_arr = precur_arr
                selfInner.lat_grid = precur_arr.latitude.values
                selfInner.lon_grid = precur_arr.longitude.values
                selfInner.area_grid = self.get_area(precur_arr) #TODO
                selfInner.grid_res = abs(selfInner.lon_grid[1] - selfInner.lon_grid[0])
        ...
        for name, filepath in list_precur_pp: # loop over all variables
            ...
            # =============================================================================
            self.calc_corr_coeffs_new(precur_arr, self.TV,
                                            lags=lags, alpha=alpha, FDR_control=FDR_control)

            # =============================================================================
            # Cluster into precursor regions
            # =============================================================================
            actor = act(name, self.xrcorr, precur_arr)

            self.outdic_precur[actor.name] = actor
        
        return self.outdic_precur

    def corr_new(self, field, ts):
        """
        ...
        self.corr_vals = np.ma.array(data = x, mask =False)
        self.pvals = np.array(x)
        fieldnans = np.array([np.isnan(field[:,i]).any() for i in range(x.size)])
        
        nonans_gc = np.arange(0, fieldnans.size)[fieldnans==False]
        
        for i in nonans_gc:
            self.corr_vals[i], self.pvals[i] = scipy.stats.pearsonr(ts,field[:,i])
    
    def func_dates_min_lag(self, dates, lag):
        ...
        self.dates_min_lag = pd.to_datetime(new_dates)
        # to be able to select date in pandas dataframe
        self.dates_min_lag_str = [d.strftime('%Y-%m-%d %H:%M:%S') for d in dates_min_lag]
    
    def calc_corr_coeffs_new(self, precur_arr, RV, lags=np.array([0]), 
                         alpha=0.05, FDR_control=True): #df_splits #TODO
        #%%
        ...
        df_splits = self.df_splits #? Wat doe je hier #TODO
        n_spl = self.df_splits.index.levels[0].size
        # make new xarray to store results
        self.xrcorr = precur_arr.isel(time=0).drop('time').copy()
        # add lags
        list_xr = [self.xrcorr.expand_dims('lag', axis=0) for i in range(n_lags)]
        self.xrcorr = xr.concat(list_xr, dim = 'lag')
        self.xrcorr['lag'] = ('lag', lags)
        # add train test split
        list_xr = [self.xrcorr.expand_dims('split', axis=0) for i in range(n_spl)]
        self.xrcorr = xr.concat(list_xr, dim = 'split')
        self.xrcorr['split'] = ('split', range(n_spl))

        print('\n{} - calculating correlation maps'.format(precur_arr.name))
        np_data = np.zeros_like(self.xrcorr.values)
        np_mask = np.zeros_like(self.xrcorr.values)
        def corr_single_split(RV_ts, precur, lags, alpha, FDR_control):
            ...
            for i, lag in enumerate(lags):
                ...
                self.func_dates_min_lag(dates_RV, lag)[1]
                prec_lag = precur_arr.sel(time=self.dates_min_lag_str)
                prec_lag = np.reshape(prec_lag.values, (prec_lag.shape[0],-1))


                # correlation map and pvalue at each grid-point:
                self.corr_new(prec_lag, RV_ts.values.squeeze())

                if FDR_control == True:
                    # test for Field significance and mask unsignificant values
                    # FDR control:
                    adjusted_pvalues = multicomp.multipletests(self.pval, method='fdr_bh')
                    ad_p = adjusted_pvalues[1]

                    self.corr_val.mask[ad_p> alpha] = True

                else:
                    self.corr_val.mask[self.pval> alpha] = True


                Corr_Coeff[:,i] = self.corr_val[:]
        ...
        RV_mask = self.df_splits.loc[0]['RV_mask']
        for s in self.xrcorr.split.values:
            progress = 100 * (s+1) / n_spl
            # =============================================================================
            # Split train test methods ['random'k'fold', 'leave_'k'_out', ', 'no_train_test_split']
            # =============================================================================
            RV_train_mask = np.logical_and(RV_mask, self.df_splits.loc[s]['TrainIsTrue'])
            RV_ts = RV.fullts[RV_train_mask.values]
            precur = precur_arr[self.df_splits.loc[s]['TrainIsTrue'].values]
        ...
        print("\n")
        self.xrcorr.values = np_data
        mask = (('split', 'lag', 'latitude', 'longitude'), np_mask )
        self.xrcorr.coords['mask'] = mask
        #%%
    
    def get_area(self, ds):
    ...
    def cluster_DBSCAN_regions(self, actor, distance_eps=400, min_area_in_degrees2=3, group_split='together'): #actor #TODO
        #%%
        """
        ...
        if group_split == 'seperate':
            for s in range(n_spl):
                ...
                grouping_split = self.mask_sig_to_cluster(mask_and_data_s, wght_area, 
        ...
        # group regions regions the same accross splits
            ...
            grouping_split = self.mask_sig_to_cluster(m_d_together, wght_area, 
                                                distance_eps, min_area_samples)
        ...
        if mask_and_data.mask.all()==True:
            ...
            if group_split == 'seperate':
                for s in range(n_spl):
                    ...
                    reassign = self.reorder_strength(prec_labels_s, corr_vals, area_grid, 
                                                min_area_samples)
                    prec_labels_ord[s] = self.relabel(prec_labels_s, reassign)
            elif group_split == 'together':
                # order based on mean corr_value:
                corr_vals = corr_xr.mean(dim='split').values
                prec_label_s = grouping_split[0].copy()
                reassign = self.reorder_strength(prec_label_s, corr_vals, area_grid, 
                                            min_area_samples)
                for s in range(n_spl):
                    prec_labels_s = prec_labels_np[s]
                    prec_labels_ord[s] = self.relabel(prec_labels_s, reassign)
    ...
    def mask_sig_to_cluster(self, mask_and_data_s, wght_area, distance_eps, min_area_samples):
        ...
        for sign in [-1, 1]:
            ...
            if len(sign_coords) != 0:
                ...
                [self.labels_sign_lag.append((l, sign)) for l in np.unique(individual_labels) if l != 0]
                ...
                pass
            self.np_regs = np.array(np_dbregs, dtype='int')
        return self.np_regs, self.labels_sign_lag
    
    def reorder_strength(self, prec_labels_s, corr_vals, area_grid, min_area_km2):
        #%%
        ...
        # self.actor.corr_strength = corr_strength
        self.reassign = {} ; key_dupl = [] ; new_reg = 0
        order_str_actor = {}
    ...
    def relabel(self, prec_labels_s, reassign):
        self.prec_labels_ord = np.zeros(prec_labels_s.shape, dtype=int)
        for i, reg in enumerate(reassign.keys()):
            self.prec_labels_ord[prec_labels_s == reg] = reassign[reg]
        return self.prec_labels_ord
    
    def get_prec_ts(self): #, outdic_precur #TODO
        ...
        for var in allvar[:]: # loop over all variables
            self.precur = self.outdic_precur[var]
            splits = self.precur.corr_xr.split
            if np.isnan(self.precur.prec_labels.values).all():
                self.precur.ts_corr = np.array(splits.size*[[]])
            else:
                self.precur.ts_corr = self.spatial_mean_regions()
                self.outdic_precur[var] = self.precur
                n_tot_regs += max([self.precur.ts_corr[s].shape[1] for s in range(splits.size)])
        return self.outdic_precur
    
    def spatial_mean_regions(self): #precur #TODO
        #%%

        var             = self.precur.name
        corr_xr         = self.precur.corr_xr
        prec_labels     = self.precur.prec_labels
        n_spl           = corr_xr.split.size
        lags = self.precur.corr_xr.lag.values

        actbox = self.precur.precur_arr.values
        self.ts_corr = np.zeros( (n_spl), dtype=object)
        ...
        for s in range(n_spl):
            ...
            df_tscorr = pd.DataFrame(tsCorr, index=pd.to_datetime(self.precur.precur_arr.time.values),
                                    columns=track_names)
            df_tscorr.name = str(s)
            self.ts_corr[s] = df_tscorr
        #%%
        return self.ts_corr

    def df_data_prec_regs(self): # TV, outdic_precur, df_splits #TODO
        #%%
        ...
        for s in range(splits.size):
            ...
            var_names_corr = [] ; actorlist = [] ; cols = [[self.TV.name]]
        
            for var in allvar[:]:
                actor = self.outdic_precur[var]
    ...
    def import_precur_ts(self, import_prec_ts, to_freq, start_end_date,
        ...
        for i, (name, path_data) in enumerate(import_prec_ts):
            ...
            if counter == 0:
                self.df_data_ext = pd.concat(list(df_data_ext_s), keys=range(splits.size))
            else:
                self.df_data_ext.merge(df_data_ext, left_index=True, right_index=True)
        return self.df_data_ext
    ...
    def get_spatcovs(self, dict_ds, df_split, s, outdic_actors, normalize=True):
        #%%
        ...
        self.df_sp_s = pd.DataFrame(data.T, index=times, columns=columns)
        dates_train = TrainIsTrue[TrainIsTrue.values].index
        s#    dates_test  = TrainIsTrue[TrainIsTrue.values==False].index
        for var, actor in outdic_actors.items():
            ...
            for i, select in enumerate(['_labels', '_labels_tigr']):
                ...
                if np.isnan(pattern.values).all():
                    ...
                    col = options[i]
                    self.df_sp_s[var + col] = pd_sp
        for i, bckgrnd in enumerate(['tcov', 'caus']):
            cols = [col for col in df_sp_s.columns if col[-4:] == bckgrnd]
            key = options[i]
            self.df_sp_s['all'+key] = df_sp_s[cols].mean(axis=1)
        #%%
        return self.df_sp_s

    def calc_spatcov(self, full_timeserie, pattern):
        #%%
        ...
        for t in range(n_time):
            ...
            self.spatcov[t] = np.cov(M)[0,1] #/ (np.sqrt(np.cov(M)[0,0]) * np.sqrt(np.cov(M)[1,1]))
        ...
        # cov xarray
        self.spatcov = xr.DataArray(self.spatcov, coords=[dates_test.values], dims=['time'])
        #%%
        return self.spatcov
...
#TODO - Oplossing zoeken voor TV.name